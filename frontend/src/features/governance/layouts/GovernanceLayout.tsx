...
export const GovernanceLayout: React.FC = () => {
  const [activeId, setActiveId] = useState('policies');
  const navigate = useNavigate();
  const location = useLocation();

  const tabs = [
    { label: 'Policies', id: 'policies', to: '/dashboard/governance/policies' },
    { label: 'Policy Acknowledgements', id: 'acknowledgements', to: '/dashboard/governance/acknowledgements' },
    { label: 'Audits', id: 'audits', to: '/dashboard/governance/audits' },
    { label: 'Compliance Issues', id: 'compliance', to: '/dashboard/governance/compliance' }
  ];

  useEffect(() => {
    // Determine the scroll container. It's the <main> element in DashboardLayout.
    const scrollContainer = document.querySelector('main');
    if (!scrollContainer) return;

    const observerOptions = {
      root: scrollContainer,
      rootMargin: '-150px 0px -40% 0px', // Triggers when the section crosses the lower-middle of the screen
      threshold: 0
    };
    ...
    // Wait a brief moment for Outlet to render the DOM elements
